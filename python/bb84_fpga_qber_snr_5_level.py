#!/usr/bin/env python3
"""
bb84_qber_curve.py — Measure QBER vs SNR on real FPGA hardware (or simulate)
════════════════════════════════════════════════════════════════════

How it works:
  1. Collect N qubits (≥3000) at one turbulence level
  2. Each qubit returns: irradiance, basis_match, data_error, lost/ok
  3. Use a sliding window (W=64 qubits) to compute local QBER and local SNR
  4. Scatter plot + smooth fit → the QBER vs SNR curve
  5. Run two passes: Fixed (SW[1]=0) vs Adaptive (SW[1]=1)
  6. Draw both curves on one plot per turbulence level
...
# ============================================================
# FPGA CONNECTION — collects real per-qubit data
# ============================================================
class FPGAConn:
    """Talks to the FPGA over UART and collects per-qubit data."""
    ...
    def collect(self, n, basis_pz=0.5, seed=42):
        """
        Collect n qubits. Returns a list of per-qubit records.

        basis_pz: probability of choosing the Z-basis (0.5=balanced, 0.8=biased)
                  Applied on the Python side because PC mode overrides the FPGA basis.

        Each record: {
            'irradiance': 0-255,      # good channel=128, fade<38
            'basis_match': 0/1,
            'data_error': 0/1,
        ...
        for i in range(n):
            # Build the qubit with the basis bias
            ad = rng.randint(0, 2)
...
# ============================================================
# SIMULATION CONNECTION — faithfully mimics the FPGA behaviour
# ============================================================
class SimConn:
    """
    Faithful simulation of the FPGA loopback:
    - Irradiance = X*Y/128, X~Gamma(α), Y~Gamma(β)
    - Deep fade: irrad < 38 → lost (timeout)
    - Outside a fade: perfect transmission (scint=0, flip=0 on the FPGA)
    """
...
def sliding_window_analysis(records, window_size=64):
    """
    Use a sliding window to compute local QBER and local SNR.

    Returns arrays: (snr_proxy, qber_pct)
    - snr_proxy: fraction of qubits received within the window (0-1) → mapped to 0-255
    - qber_pct: QBER (%) computed over that window

    How QBER accounts for lost qubits:
      - Lost qubit → basis_match~50%, error~50% → QBER~50% over the lost fraction
...
def plot_level(level, snr_f, qber_f, irrad_f, snr_a, qber_a, irrad_a,
    ...
    """Plot QBER vs SNR for one turbulence level: Fixed (red) vs Adaptive (cyan)."""
    p = TURB[level]
...
""")
    ...
    for lv in levels:
        ...
        for adaptive_on in [False, True]:
            ...
            if adaptive_on:
                # Adaptive: use a basis bias suited to the turbulence level
