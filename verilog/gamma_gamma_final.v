...
//
// MODEL PARAMETERS (Al-Habash / Andrews / Phillips, 2001):
...
module gamma_gamma_channel (
    ...
    // Instead of outputting 1'b0 on a fade, use this random bit (as in Python)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // [FIX v2] Resample X per qubit (as in Python: x1 = np.random.gamma() each time)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // [FIX v2] Resample Y per qubit (as in Python: x2 = np.random.gamma() each time)
    ...
    // [MATCH PYTHON v3] scint_thresh = 0 → scintillation blanking disabled
    // Python has no such mechanism → disabled so the FPGA matches Python
    ...
    // [MATCH PYTHON v3] do_flip = 0 → only deep_fade causes random errors
    // Python model: if fading < threshold → random bit, otherwise perfect transmission
    // The original Verilog also added irradiance-driven BER (do_flip) → QBER ~40% vs Python ~3%
    // do_flip is disabled so the FPGA result matches the Python simulation
    ...
    //
    // [FIX v2]: fade/scint → random bit (instead of 1'b0)
    // Python: if fading < threshold → return random.choice(['0','1'])
    // The original Verilog was wrong: always output 0 → systematically wrong QBER
    ...
    always @(*) begin
        if (!turb_enable)
            ...
            signal_out_reg = lfsr_fade_rand[0];   // [FIX] Random bit (as in Python)
        else if (scint_blank)
