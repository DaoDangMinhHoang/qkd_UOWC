    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!window_valid) begin
                ...
                //   indicator is NO CLICKS AT ALL.
                if (photon_rate == 8'd0) begin
                    // ---- ESCAPE HATCH FOR THE WAVELENGTH HILL-CLIMBER ----
                    // The λ hill-climber can trap itself: if it wanders onto
                    // a λ far too poor for the current water (e.g. picking
                    // 650 nm in clear ocean, ~270× worse attenuation) the
                    // link DIES ⇒ window_valid = 0 ⇒ the λ FSM (which sits
                    // inside the window_valid branch) NEVER RUNS AGAIN.
                    // Found by tb_adaptive_loop.v TEST C.
                    // Fix: before declaring the link lost, try the remaining
                    // λ one by one (at stale = 2, 4, 6 → covers all 3).
                    if (stale_windows != 8'd0 && stale_windows[0] == 1'b0
                        && stale_windows < DEAD_WINDOWS) begin
                        lam_best   <= lam_up;
                        lambda_sel <= lam_up;
                        lam_state  <= LAM_SETTLE;
                        lam_timer  <= 4'd0;
                    end
                    // All λ tried and still no clicks ⇒ the link really is lost.
