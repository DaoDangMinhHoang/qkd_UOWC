...
// ============================================================
//
// Wraps trng.v behind an lfsr_random-compatible interface:
//   - random_bit: 1-bit output (truly random)
//   - random_byte: 8-bit output (truly random)
//   - enable: advance signal (latch new values)
//
// The TRNG runs continuously, accumulating bits into a shift register.
// On an enable pulse the current value is latched to the output.
//
// Throughput: the TRNG produces ~97k bits/s (>> the 200 bits/s BB84 needs)
// → there is always enough entropy on hand.
//
// The SEED parameter is kept for backward compatibility but
// is unused (a TRNG needs no seed).
// ============================================================
